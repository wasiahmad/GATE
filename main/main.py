...
def main(args):
    ...
    if args.only_test:
        ...
        if args.checkpoint and os.path.isfile(args.model_file + '.checkpoint'):
            ...
            if args.pretrained:
                ...
                model = EventRelationExtractor.load(args.pretrained, args)
            else:
                logger.info('Training model from scratch...')
                # Build a dictionary from the data questions + words (train/dev splits)
                logger.info('-' * 100)
                # Initialize model
                model = EventRelationExtractor(config.get_model_args(args),
    ...
    if not model.args.use_bert:
        args.bert_train_file = [None] * len(args.language)
        args.bert_valid_file = [None] * len(args.language)

    train_exs = []
    dataset_weights = dict()
    if not args.only_test:
        for train_file, bert_train_file, lang in \
                zip(args.train_file, args.bert_train_file, args.language):
            exs = util.load_data(train_file,
                                 lang,
                                 bert_feat_file=bert_train_file,
    ...
    dev_exs = []
    for valid_file, bert_valid_file, lang in \
            zip(args.valid_file, args.bert_valid_file, args.language):
        exs = util.load_data(valid_file,
                             lang,
                             bert_feat_file=bert_valid_file,
                             max_examples=args.max_examples)
        dev_exs.extend(exs)
    ...
    train_loader = None
    if not args.only_test:
        train_dataset = data.ACE05Dataset(train_exs, model, evaluation=False)
        if args.sort_by_len:
    ...
    dev_dataset = data.ACE05Dataset(dev_exs, model, evaluation=True)
