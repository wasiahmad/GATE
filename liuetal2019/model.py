...
from clie.model import ClTransClassifier

logger = logging.getLogger(__name__)


class CL_TRANS_GCN(object):
    ...
    def __init__(self, args, state_dict=None, vocab_file=None):
        # Book-keeping.
        self.name = 'CL_TRANS_GCN'
        self.args = args
        self.updates = 0
        self.use_cuda = False
        self.parallel = False
        self.args.bert_dim = 0
        if vocab_file:
            self.load_vocabulary(vocab_file)

        self.network = ClTransClassifier(self.args)
    ...
    def load_vocabulary(self, vocab_file):
        ...
        logger.info('Num words = %d, label = %d' %
                    (self.args.word_size, self.args.label_size))
    ...
    def update(self, ex):
        ...
        if self.use_cuda:
            ...
            knn_rep = knn_rep.cuda(non_blocking=True)
        ...
        # Run forward
        main_out, knn_out = self.network(len_rep=len_rep,
                                         word_rep=word_rep,
                                         pos_rep=pos_rep,
                                         ner_rep=ner_rep,
        ...
        main_loss = self.criterion(main_out, labels) * ex_weights
        main_loss = main_loss.mean()
        knn_loss = self.criterion(knn_out, labels) * ex_weights
        knn_loss = knn_loss.mean()
        loss = main_loss + self.args.alpha * knn_loss
    ...
    def predict(self, ex):
        ...
        if self.use_cuda:
            ...
            labels = labels.cuda(non_blocking=True)
            knn_rep = knn_rep.cuda(non_blocking=True) \
                if knn_rep is not None else knn_rep
